...
const AdjustableNumber = (props : AdjustableNumberProps) => {
    ...
    return <div className={Styles.AdjustableContainer}>
        {displayMode === DISPLAYMODE.EDIT && <button onClick={() => setDisplayNumber(displayNumber - 1)}>&larr;</button>}
        <span
            className={Styles.AdjustableNumber}
            onDoubleClick={() => setDisplayMode(displayMode === DISPLAYMODE.DISPLAY
            ? DISPLAYMODE.EDIT
            : DISPLAYMODE.DISPLAY)}>
            {displayNumber}
        </span>
        {displayMode === DISPLAYMODE.EDIT && <button onClick={() => setDisplayNumber(displayNumber + 1)}>&rarr;</button>}
    </div>
};
